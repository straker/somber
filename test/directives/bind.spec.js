...
import { setupFixture, wait } from '../testutils.js';
...
describe('bind directive', () => {
  ...
  describe('class binding', () => {
    describe('sets the class', () => {
      it('when true', () => {
        const { target } = setupFixture(`<div id="target" :class="{ hello: true }">hello</div>`);
        assert.isTrue(target.classList.contains('hello'));
      });

      it('when expression is true', () => {
        const { target } = setupFixture(`<div id="target" :class="{ hello: true == 1 }">hello</div>`);
        assert.isTrue(target.classList.contains('hello'));
      });

      it('when expression is truthy', () => {
        const { target } = setupFixture(`<div id="target" :class="{ hello: 'foo' }">hello</div>`);
        assert.isTrue(target.classList.contains('hello'));
      });

      it('when state is true', () => {
        const { target } = setupFixture(`<div id="target" :class="{ hello: state.value }">hello</div>`, {
          state: {
            value: true
          }
        });
      ...
      it('when multiple properties are true', () => {
        const { target } = setupFixture(`<div id="target" :class="{ hello: 'foo', world: true }">hello</div>`);
        assert.isTrue(target.classList.contains('hello'));
        assert.isTrue(target.classList.contains('world'));
      });

      it('when some properties are true and some false', () => {
        const { target } = setupFixture(`<div id="target" :class="{ hello: 'foo', world: false, foo: 1 }">hello</div>`);
        assert.isTrue(target.classList.contains('hello'));
      ...
      it('when the binding changes', () => {
        const { target, host } = setupFixture(`<div id="target" :class="{ hello: state.value }">hello</div>`, {
          state: {
            value: false
          }
        });
      ...
      it('does not remove static class values', () => {
        const { target } = setupFixture(`<div id="target" class="exists" :class="{ hello: true }">hello</div>`);
    ...
    describe('removes the class', () => {
      it('when false', () => {
        const { target } = setupFixture(`<div id="target" :class="{ hello: false }">hello</div>`);
        assert.isFalse(target.classList.contains('hello'));
      });

      it('when undefined', () => {
        const { target } = setupFixture(`<div id="target" :class="{ hello: undefined }">hello</div>`);
        assert.isFalse(target.classList.contains('hello'));
      });

      it('when null', () => {
        const { target } = setupFixture(`<div id="target" :class="{ hello: null }">hello</div>`);
        assert.isFalse(target.classList.contains('hello'));
      });

      it('when ""', () => {
        const { target } = setupFixture(`<div id="target" :class="{ hello: '' }">hello</div>`);
        assert.isFalse(target.classList.contains('hello'));
      });

      it('when expression is false', () => {
        const { target } = setupFixture(`<div id="target" :class="{ hello: false == 1 }">hello</div>`);
        assert.isFalse(target.classList.contains('hello'));
      });

      it('when expression is falsey', () => {
        const { target } = setupFixture(`<div id="target" :class="{ hello: true ? 0 : 1 }">hello</div>`);
        assert.isFalse(target.classList.contains('hello'));
      });
      ...
      it('when multiple properties are false', () => {
        const { target } = setupFixture(`<div id="target" :class="{ hello: false, world: undefined }">hello</div>`);
        assert.isFalse(target.classList.contains('hello'));
        assert.isFalse(target.classList.contains('world'));
      });

      it('when some properties are false and some true', () => {
        const { target } = setupFixture(`<div id="target" :class="{ hello: '', world: true, foo: false }">hello</div>`);
        assert.isFalse(target.classList.contains('hello'));
        assert.isTrue(target.classList.contains('world'));
      ...
      it('does not remove static class values', () => {
        const { target } = setupFixture(`<div id="target" class="exists" :class="{ hello: false }">hello</div>`);
        assert.isTrue(target.classList.contains('exists'));
      });

      it('removes static class values when same name', () => {
        const { target } = setupFixture(`<div id="target" class="exists" :class="{ exists: false }">hello</div>`);
  ...
  describe('style binding', () => {
    describe('sets the style', () => {
      it('to value', () => {
        const { target } = setupFixture(`<div id="target" :style="{ width: '100px' }">hello</div>`);
        assert.equal(target.style.width, '100px');
      });

      it('to falsey value', () => {
        const { target } = setupFixture(`<div id="target" :style="{ border: 0 }">hello</div>`);
        assert.equal(target.style.border, '0px');
      });

      it('to expression result', () => {
        const { target } = setupFixture(`<div id="target" :style="{ position: true ? 'relative' : 'false' }">hello</div>`);
        assert.equal(target.style.position, 'relative');
      });

      it('to state', () => {
        const { target } = setupFixture(`<div id="target" :style="{ padding: state.value }">hello</div>`, {
          state: {
            value: '10px'
          }
        });
      ...
      it('when multiple properties have values', () => {
        const { target } = setupFixture(`<div id="target" :style="{ border: '10px', 'font-size': '12px' }">hello</div>`);
        assert.equal(target.style.border, '10px');
        assert.equal(target.style['font-size'], '12px');
      });

      it('when some properties are true and some false', () => {
        const { target } = setupFixture(`<div id="target" :style="{ border: '10px', padding: false, 'font-size': '12px' }">hello</div>`);
        assert.equal(target.style.border, '10px');
      ...
      it('when the binding changes', () => {
        const { target, host } = setupFixture(`<div id="target" :style="{ padding: state.value }">hello</div>`, {
          state: {
            value: false
          }
        });
      ...
      it('does not remove static style values', () => {
        const { target } = setupFixture(`<div id="target" style="padding: 2px" :style="{ border: 0 }">hello</div>`);
    ...
    describe('removes the style', () => {
      it('when false', () => {
        const { target } = setupFixture(`<div id="target" :style="{ width: false }">hello</div>`);
        assert.equal(target.style.width, '');
      });

      it('when undefined', () => {
        const { target } = setupFixture(`<div id="target" :style="{ width: undefined }">hello</div>`);
        assert.equal(target.style.width, '');
      });

      it('when null', () => {
        const { target } = setupFixture(`<div id="target" :style="{ width: null }">hello</div>`);
        assert.equal(target.style.width, '');
      });

      it('when ""', () => {
        const { target } = setupFixture(`<div id="target" :style="{ width: '' }">hello</div>`);
        assert.equal(target.style.width, '');
      });

      it('when expression is false', () => {
        const { target } = setupFixture(`<div id="target" :style="{ position: false === 1 }">hello</div>`);
        assert.equal(target.style.position, '');
      });

      it('when expression is falsey', () => {
        const { target } = setupFixture(`<div id="target" :style="{ 'line-height': true ? undefined : true }">hello</div>`);
      ...
      it('when multiple properties are false', () => {
        const { target } = setupFixture(`<div id="target" :style="{ border: '', 'font-size': null }">hello</div>`);
        assert.equal(target.style.border, '');
        assert.equal(target.style['font-size'], '');
      });

      it('when some properties are false and some true', () => {
        const { target } = setupFixture(`<div id="target" :style="{ border: null, padding: '2px', 'font-size': false }">hello</div>`);
        assert.equal(target.style.border, '');
        assert.equal(target.style.padding, '2px');
        assert.equal(target.style['font-size'], '');
      });
      ...
      it('removes static style values when same name', () => {
        const { target } = setupFixture(`<div id="target" style="border: 10px" :style="{ border: false }">hello</div>`);
        assert.equal(target.style.border, '');
      });
    });
  });
