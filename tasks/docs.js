...
const excludeCodeRegex =
  /\s*\/\/ exclude-code:start[\s\S]*?\/\/ exclude-code:end/g;
const excludeScriptRegex =
  /\s*\/\/ exclude-script:start[\r\n]([\s\S]*?[\r\n])\/\/ exclude-script:end[\r\n]/g;
const codeRegex =
  /<pre>[\s\S]*?<code class="(.*)">([\s\S]*?)<\/code><\/pre>/g;
...
function parseType(type) {
  ...
  type = type.replace(
    '*',
    type.startsWith('*') ? 'Any type' : 'any type'
  );
...
let tags = {
  ...
  docs: function () {
    ...
    let parseComments = require(
      path.join(
        require.resolve('livingcss'),
        '../lib/parseComments.js'
      )
    );
    let livingCSSTags = require(
      path.join(require.resolve('livingcss'), '../lib/tags.js')
    );
    let tagList = {
      ...livingCSSTags,
  ...
  try {
    if (context.navbar) {
      ...
      context.navbar.forEach(navItem => {
        ...
        else if (navItem.url === 'spritesheet')
          navItem.url = 'spriteSheet';
        else if (navItem.url === 'tileengine')
          navItem.url = 'tileEngine';
        else if (navItem.url === 'gameobject')
          navItem.url = 'gameObject';
      });
...
function buildServiceWorker() {
  ...
  return gulp
    ...
    .pipe(
      replace(staticCacheName, (match, p1) => {
...
gulp.task(
  'build:docs',
  gulp.series(buildApi, buildPages, buildServiceWorker)
);
